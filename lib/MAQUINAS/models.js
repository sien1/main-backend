...
exp.saveMaquinaInfo = async(info) => {
    try {
        ...
        const insertString = _makeInsertString(info.data);
        ...
        if(verifyMaquinaCliente.recordset[0].length > 0)
            return;
    ...
    //obtener los datos que vas a actualizar
    //await db.procedure('Maquinas.dbo.UPDATE_MAQUINA_INFO', [codigo, usuario]);
