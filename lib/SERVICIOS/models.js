...
exp.insertarServicio = async(params) => {
    ...
    debugger;
        ...
        try{
            ...
            const resultado = await db.procedure('Servicios.dbo.Servicios', Servicios);
...
exp.getServiciosByProyecto = async (IdProyecto) => {
    try {
        const query = `
            SELECT servs.Id, emps.Nombre+' '+emps.ApellidoPaterno as Nombre, 
            CONVERT(VARCHAR,servs.FechaAlta,3) as 'Fecha Alta',
            tipoServ.Descripcion as Categoria,
            cats.Descripcion,
            servs.Descripcion as Indicaciones
            FROM Servicios.dbo.Servicios servs
            LEFT JOIN Servicios.dbo.CatalogoServicios cats ON cats.Id = servs.IdCatServicios
            LEFT JOIN Servicios.dbo.CatalogoTiposServicio tipoServ ON cats.IdTipoServicio = tipoServ.Id 
            LEFT JOIN Empleados.dbo.Usuarios usu ON usu.Id = servs.IdUsuarioAlta
            LEFT JOIN Empleados.dbo.Empleados emps ON emps.Id = usu.IdEmpleado
            WHERE servs.IdProyecto = ${IdProyecto}`;
